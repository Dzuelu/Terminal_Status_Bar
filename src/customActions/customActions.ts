...
class CustomActions {
  ...
  private registerStatusBarCommand = (
    name: string,
    path: string,
    commandToRun: string
  ) => {
    if (!this.commands[this.calculateTheCommandName(name)]) {
      vscode.commands.registerCommand(
        this.calculateTheCommandName(name),
        () => {
  ...
  private makecommand(name: string, path: string, commandToRun: string) {
    this.commands[this.calculateTheCommandName(name)] = this.callbackCommand.bind(
      this,
      name,
      path,
      commandToRun
